...
class Giocatore {
    ...
    algo2() {
        ...
        // ...altrimenti, cerca la mossa migliore
            ...
            return this.analizza2(possibili)
        }
    ...
    analizza2(possibili) {
        var prese = [], perse = []
        for (var i in possibili)
            possibili[i].prendo ? prese.push(possibili[i]) : perse.push(possibili[i])
        ...
        // un carico perso è considerato di maggior valore di una briscola minore
        perse.sort( (a,b) => (a.suoi_punti + (a.carta[1] == this.partita.briscola[1]? 5:0)) - (b.suoi_punti + (b.carta[1] == this.partita.briscola[1]? 5:0)))
...
class Partita {
    constructor() {
        ...
        this.briscola = this.mazzo.briscola()
        ...
        for (var i=0; i < this.giocatori.length; i++) {
            this.giocatori[i].mano = this.mazzo.prendi(3)
        }
    
        // mostra le carte
        var doc = document.querySelectorAll('#Giocatore1 > img')
        for (var i=0; i < doc.length; i++) doc[i].src = 'trieste/dorso.png'
        doc = document.querySelectorAll('#Giocatore2 > img')
        for (i=0; i < doc.length; i++) { 
            doc[i].src = 'trieste/'+this.giocatori[1].mano[i]+'.png' // carica la carta
            doc[i].setAttribute('onclick', 'partita.gioca(1,'+i+')') // assegna un evento onclick
        }
        // mostra la briscola
        document.querySelectorAll('#Banco > img')[2].src = 'trieste/'+this.briscola+'.png'
        console.log('La briscola è ' + this.mazzo.nome(this.briscola).split(' ').slice(-1))
        this.info('La briscola è ' + this.mazzo.nome(this.briscola).split(' ').slice(-1))
    }
    ...
    gioca(giocatore, carta=-1) {
        ...
        if (giocatore == 1) {
            ...
            document.querySelectorAll('#Giocatore2 > img')[giocata.i].style.visibility = 'hidden'
            document.querySelectorAll('#Banco > img')[0].src = 'trieste/'+giocata.nome+'.png'
            document.querySelectorAll('#Banco > img')[0].style.visibility = 'visible'
            ...
            document.querySelectorAll('#Giocatore1 > img')[giocata.i].style.visibility = 'hidden'
            document.querySelectorAll('#Banco > img')[1].src = 'trieste/'+giocata.nome+'.png'
            document.querySelectorAll('#Banco > img')[1].style.visibility = 'visible'
        }
        ...
        if (this.carte_giocate.length == 2) {
            ...
            // se è maggiore la carta dell'umano, o era lui di mano... 
                ...
                this.info('Prende UMANO. '+this.giocatori[1].punti+' a '+this.giocatori[0].punti)
                if (! this.mazzo.vuoto()) {
                    this.giocatori[1].prendi_carta(this.mazzo.pesca())
                    this.giocatori[0].prendi_carta(this.mazzo.pesca())
                }
            ...
            if (this.mani.length == 40) {
                ...
                else if (this.giocatori[0].punti >  this.giocatori[1].punti) msg += "Vince PC: " + this.giocatori[0].punti+'-'+this.giocatori[1].punti
                else msg += "Vinci TU: " + this.giocatori[1].punti+'-'+this.giocatori[0].punti
                this.info(msg)
                return
            }
    ...
    rinfresca_carte() {
        ...
        document.querySelectorAll('#Banco > img')[0].style.visibility = 'hidden'
        // nasconde la carta giocata dal PC se è di mano o sono finite le carte
        if (this.primo_di_mano == 1 || ! this.giocatori[0].mano.length)
            document.querySelectorAll('#Banco > img')[1].style.visibility = 'hidden'
        // nasconde la briscola una volta pescata a fine mazzo
        if (this.mazzo.vuoto()) document.querySelectorAll('#Banco > img')[2].style.visibility = 'hidden'
        // rinfresca le mani
        for (var i=0; i < 3; i++) {
            var m
            m = this.giocatori[0].mano[i]
            document.querySelectorAll('#Giocatore1 > img')[i].style.visibility = (m != undefined)? 'visible':'hidden'
            m = this.giocatori[1].mano[i]
            if (m != undefined) {
                document.querySelectorAll('#Giocatore2 > img')[i].src =  'trieste/'+m+'.png' // carica il file della carta
                document.querySelectorAll('#Giocatore2 > img')[i].style.visibility = 'visible'
            }
            else
                document.querySelectorAll('#Giocatore2 > img')[i].style.visibility = 'hidden'
        }
    }

    info(s) {
        document.getElementById('riga').innerHTML = s
    }
};
