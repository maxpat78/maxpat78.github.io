...
class Tavolo {
    constructor() {
        this.width = 0
        this.height = 0
        this.imgs = [] // ipxagini di ogni carta del mazzo
        this.gfx_mazzo = [] // ipxagini dei dorsi e della briscola componenti il mazzo
        this.gfx_manopc = [] // ipxagini della mano del PC
        this.gfx_manome = [] // ipxagini della mano umana
        this.gfx_banco = [] // ipxagini del banco
    ...
    // precarica le ipxagini di tutte le carte, la prima volta
    carica_carte() {
    ...
    disegnaMazzo() {
        // posizione iniziale (in % della viewport)
        var x = 10, y = 35
    ...
    disegnaCarte() {
        ...
        for(var i=0; i<3; i++) {
            ...
            img.style.position = 'absolute'
            img.style.left = x_pc+'vw'
            img.style.top = y_pc+'vh'
            if (DEBUG) console.log('.left .top carta pc:',x_pc,y_pc)
            //~ x_pc += 5
            x_pc += ((0.6*this.gfx_mazzo[1].width)/this.width*100.0)
            console.log(x_pc, img.width, this.width)
        ...
        for(var i=0; i<3; i++) {
            ...
            img.height *= this.ratio
            img.style.position = 'absolute'
            img.style.left = x_me+'vw'
            img.style.top = y_me+'vh'
            if (DEBUG) console.log('.left .top carta mia:',x_me,y_me)
            //~ x_me += 5
            x_me += ((0.6*this.gfx_mazzo[1].width)/this.width*100.0)
    ...
    daiCarta(giocatore, indice, tempo=1000) {
        ...
        this.animaIpxagine(img, {x: gfx[indice].x, y: gfx[indice].y}, tempo)
        if (giocatore) {
    ...
    animaIpxagine(img, puntoB, durata, passo=0.01) {
        var progresso=0
        this.animazione = 1
    ...
    gioca(giocatore, indice) {
        ...
        this.animaIpxagine(gfx[indice+10], {x: this.gfx_banco[giocatore].x, y: this.gfx_banco[giocatore].y}, 1500)
    ...
    arbitra() {
        ...
        if (r < 0) {
            ...
            // anima la presa
            this.animaIpxagine(c1, {x: c1.x, y: c1.y + 1.75*c1.height}, 500)
            this.animaIpxagine(c2, {x: c2.x, y: c2.y + 1.75*c2.height}, 500)
